...
// Presentational job detail. Data is fetched on the server (see page.tsx) and
// passed in, so the JobPosting JSON-LD and metadata are in the initial HTML.
export function JobDetailClient({ job }: { job: JobView }) {
        ...
        <div className="min-h-screen bg-background text-foreground relative overflow-hidden flex flex-col font-sans transition-colors duration-300">
            ...
            <header className="relative border-b border-sidebar-border backdrop-blur-md bg-sidebar-bg/85 py-4.5 z-10 shrink-0">
                <div className="max-w-4xl mx-auto px-4 flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-4">
                        <button onClick={toggleTheme} className="p-2 rounded-xl text-slate-500 hover:bg-slate-200/50 dark:hover:bg-white/[0.05] transition-all" title={`Switch to ${theme === "dark" ? "Light" : "Dark"} Mode`}>
                            {theme === "dark" ? <Sun className="h-4.5 w-4.5 text-amber-500" /> : <Moon className="h-4.5 w-4.5 text-indigo-500" />}
                        </button>

                        <button type="button" onClick={() => router.push(`/jobs/${job._id}/apply`)} className="bg-brand-primary hover:bg-brand-primary-dark text-white font-bold text-sm px-4 py-2.5 rounded-xl transition-all shadow-lg shadow-brand-primary/20 flex items-center gap-1.5 active:scale-95">
            ...
            <section className="relative z-10 border-b border-sidebar-border overflow-hidden">
                ...
                <div className="max-w-5xl mx-auto px-4 py-10 md:py-14 relative">
                    <button onClick={() => router.push("/jobs")} className="flex items-center gap-1.5 text-sm font-bold text-text-muted hover:text-text-primary transition-all mb-6">
            ...
            <main className="relative flex-1 max-w-5xl w-full mx-auto px-4 py-10 z-10 grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                ...
                <div className="lg:col-span-2 space-y-4">
                    {job.sections && job.sections.length > 0 ? (
                        job.sections.map((sec, idx) => (
                            <div key={idx} style={{ animationDelay: `${idx * 60}ms` }} className="animate-fade-up crm-panel p-6 rounded-2xl space-y-2.5">
                ...
                <aside className="lg:sticky lg:top-24 space-y-4">
                    ...
                    <div className="crm-panel rounded-2xl p-6 space-y-4 shadow-xl">
                        ...
                        <button type="button" onClick={() => router.push(`/jobs/${job._id}/apply`)} className="w-full bg-brand-primary hover:bg-brand-primary-dark text-white font-bold text-sm py-3 rounded-xl shadow-lg shadow-brand-primary/20 flex items-center justify-center gap-2 active:scale-[0.98] transition-all">
                            Apply Now <ArrowRight className="h-4 w-4" />
                        </button>
