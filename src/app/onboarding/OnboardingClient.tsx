...
// Shared onboarding self-service UI. `trackApiBase` is the subject's track
// endpoint — `/api/onboarding/track/<applicationId>` for an applicant, or
// `/api/onboarding/track/by-record/<recordId>` for a staff member (Phase 3).
export default function OnboardingClient({ trackApiBase }: { trackApiBase: string }) {
    ...
    const load = async () => {
        ...
        try {
            const res = await fetch(`${trackApiBase}?${cred}`);
    ...
    useEffect(() => {
        if (trackApiBase && email && token) load();
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [trackApiBase, email, token]);
    ...
    const persist = async (submit: boolean) => {
        ...
        try {
            ...
            const res = await fetch(trackApiBase, {
